...
async def folder_make(body: schema.FolderMake):
    DB = await aioredis.Redis(connection_pool=await function.pool(function.FOLDER_DB))
    ...
    if body.folder_password is None:
        DB_SALT = aioredis.Redis(connection_pool=await function.pool(function.SALT_DB))
...
async def folder_open(folder_id: str):
    DB = aioredis.Redis(connection_pool=await function.pool(function.FOLDER_DB))
