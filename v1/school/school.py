...
async def meal_info(body: SchemaMealInfo):
    ...
    print(sd_schul_code)
    url = f"https://open.neis.go.kr/hub/mealServiceDietInfo?KEY={OPEN_NEIS_API_KEY}&Type=json&pIndex=1&pSize=10&ATPT_OFCDC_SC_CODE={atpt_ofcdc_sc_code}&SD_SCHUL_CODE={sd_schul_code}&MLSV_FROM_YMD={DATE}&MLSV_TO_YMD={DATE}"
    print(url)
    request = requests.get(url=url)
